"""
AlJazeeraScraper — Scraper for Al Jazeera English.

Al Jazeera is an international news network headquartered in Qatar.
This scraper targets the English version (aljazeera.com).
"""
...
class AlJazeeraScraper(BaseScraper):
    """
    Specific scraper for Al Jazeera English.
    Extracts articles from the main news section.
    """
    ...
    def get_article_urls(self) -> list[str]:
        """
        Extracts article URLs from the news page.
        Al Jazeera uses paths like /news/YYYY/M/D/slug.
        """
        urls = []
        ...
        self.logger.info("Al Jazeera URLs found", extra={"count": len(urls)})
        return urls[:20]

    def parse_article(self, url: str, soup: BeautifulSoup) -> Optional[dict]:
        """Extracts data from an Al Jazeera article."""
        article = {}
