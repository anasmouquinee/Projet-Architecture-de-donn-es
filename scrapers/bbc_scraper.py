"""
BBCScraper — Scraper for BBC News.

BBC News is one of the largest international news sources.
This scraper targets the main English-language news section.
"""
...
class BBCScraper(BaseScraper):
    """
    Specific scraper for BBC News (www.bbc.com/news).

    Extracts articles from the main news page
    ...
    def get_article_urls(self) -> list[str]:
        """
        Extracts article links from the BBC News home page.

        BBC uses a predictable URL format for articles:
        /news/articles/<id> or /news/<section>-<id>
        """
        urls = []
        ...
        self.logger.info("BBC URLs found", extra={"count": len(urls)})
        return urls[:20]  # Limit to 20 articles per run

    def parse_article(self, url: str, soup: BeautifulSoup) -> Optional[dict]:
        """
        Extracts data from a BBC News article.

        BBC has a relatively consistent structure:
        - Title: main <h1> of the article
        - Author: byline with journalist name
        - Date: <time> element with a datetime attribute
        - Content: text blocks inside the article body
        - Category: section tag in breadcrumb or Open Graph metadata
        """
        article = {}
