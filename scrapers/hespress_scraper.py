"""
HespressScraper — Scraper for the Moroccan news site Hespress.

Hespress is one of the largest news sites in Morocco.
It publishes in Arabic and French. This scraper targets the French version.
"""
...
class HespressScraper(BaseScraper):
    """
    Specific scraper for Hespress (fr.hespress.com).

    Extracts articles from the home page and their detail pages.
    """
    ...
    def get_article_urls(self) -> list[str]:
        """
        Extracts article links from the home page.

        Looks for links inside article cards in the main
        ...
        self.logger.info("Hespress URLs found", extra={"count": len(urls)})
        return urls[:20]  # Limit to 20 articles per run

    def parse_article(self, url: str, soup: BeautifulSoup) -> Optional[dict]:
        """
        Extracts data from a Hespress article.

        Typical Hespress structure:
        - Title: <h1> inside the article header
        - Author: span with an author class
        - Date: time element or span with date
        - Content: div with the article body
        - Category: breadcrumb or section tag
        """
        article = {}
