"""
run_scrapers.py — Main entry point for running all scrapers.

This script runs all registered scrapers, collects the articles,
and saves them as JSON. Can be invoked directly or from Airflow.

Usage:
    python run_scrapers.py                  # Run all scrapers
    python run_scrapers.py --source bbc     # Run only BBC
    python run_scrapers.py --output ./out   # Custom output directory
"""
...
def run_scrapers(sources: list[str] | None = None) -> list[dict]:
    """
    Runs the requested scrapers and returns all collected articles.

    Args:
        sources: List of source names to run.
                 If None, runs all registered scrapers.
    """
    if sources is None:
    ...
    for source_name in sources:
        ...
        if scraper_class is None:
            logger.warning("Source not registered, skipping", extra={"source": source_name})
            continue

        logger.info("Running scraper", extra={"source": source_name})
        try:
            ...
            logger.info(
                "Scraper completed",
                extra={"source": source_name, "count": len(articles)}
            )
        except Exception as error:
            logger.error(
                "Scraper error",
                extra={"source": source_name, "error": str(error)}
            )
...
def save_to_file(articles: list[dict], output_dir: str) -> str:
    """
    Saves articles to a timestamped JSON file.
    ...
    logger.info("Articles saved", extra={"path": filepath, "count": len(articles)})
    return filepath
...
def main():
    ...
    if articles:
        ...
        print(f"\n[OK] {len(articles)} articles saved to: {filepath}")
    else:
        print("\n[WARN] No articles were collected.")
        sys.exit(1)
