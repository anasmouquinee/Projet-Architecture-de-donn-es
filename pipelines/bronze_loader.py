"""
bronze_loader.py — Loads raw articles into the Bronze bucket in MinIO.

This module is the entry point for batch ingestion:
it receives articles from the scraper and stores them in the Data Lake
without any transformation (raw data).
"""
...
class BronzeLoader:
    """Loads raw data into the Bronze layer of the Data Lake (MinIO)."""
    ...
    def load_articles(self, articles: list[dict]) -> int:
        """
        Loads a list of articles into Bronze.
        Returns the number of articles loaded successfully.
        """
        success_count = 0
        ...
        for article in articles:
            try:
                ...
                logger.error(
                    ...
                    extra={"url": article.get("url"), "error": str(error)}
                )

        logger.info(
            "Bronze load completed",
            extra={"loaded": success_count, "total": len(articles)}
        )
        return success_count

    def load_batch_file(self, filepath: str) -> int:
        """
        Loads a complete JSON file as a batch into Bronze.
        Also saves the full file as a backup.
        """
