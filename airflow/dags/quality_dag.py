"""
quality_dag.py — Data quality checks DAG.

Runs completeness, coherence, and validity checks
on processed articles. Generates reports and moves
problematic articles to quarantine.
"""
...
def task_run_quality_checks(**context):
    """
    Runs quality checks on the Silver articles for the day.
    Reads the Silver Parquet, runs the checks, and generates a report.
    """
    import boto3
    import pandas as pd
...
def task_save_quality_report(**context):
    """Saves the quality report to MinIO."""
...
def task_quarantine_bad_records(**context):
    """
    Moves articles with a low quality score to quarantine.
    These articles must not reach the Data Warehouse.
    """
    import boto3
    ...
    # Log quarantined URLs for reference
    s3 = boto3.client(
        "s3",
        endpoint_url=os.getenv("MINIO_ENDPOINT", "http://minio:9000"),
        aws_access_key_id=os.getenv("MINIO_ROOT_USER", "minio_admin"),
