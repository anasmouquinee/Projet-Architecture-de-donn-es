"""
scraping_dag.py — Batch scraping DAG scheduled every hour.

This DAG runs all registered scrapers, collects articles,
and loads them into the Bronze layer of the Data Lake (MinIO).
It also publishes the articles to Kafka for the streaming flow.
"""
...
def task_scrape_articles(**context):
    """Runs all scrapers and returns the collected articles."""
...
def task_load_to_bronze(**context):
    """Loads the scraped articles into the Bronze layer of MinIO."""
...
def task_publish_to_kafka(**context):
    """Publishes the articles to Kafka for the streaming flow."""
